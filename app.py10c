...
# =========================================================
# Rota Generator — Clean Rebuild (v10c)
# =========================================================
# NO Parameters sheet required.
...
def site_allowed(sr, role: str, d: date, allow_cross: bool) -> bool:
    ...
    if role == "Bookings":
        return home == "SLGP"
    if role.startswith("Triage_Admin"):
        return (home == role_site(role, d)) or allow_cross
    if role == "Awaiting_PSA_Admin":
        return (home == awaiting_site_for_day(d)) or allow_cross
        ...
        for site in ["SLGP", "JEN", "BGS"]:
            ...
            for name, sr in staff_by_name.items():
                if str(sr.get("HomeSite", "")).upper() != site:
                    continue
                if not candidate_ok(name, role, d, t, allow_cross=False):
                    continue
...
st.set_page_config(page_title="Rota Generator (Clean Rebuild)", layout="wide")
require_password()

st.title("Rota Generator — Clean Rebuild")
