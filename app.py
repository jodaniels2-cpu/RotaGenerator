...
# If a weight column is missing, we default to 3 (neutral).
# Expected optional columns on Staff sheet (0–5):
#   PhonesWeight, BookingsWeight, EmisWeight, DocmanWeight, AwaitingWeight, EmailWeight, TriageWeight, FrontDeskWeight
                ...
                if not chosen:
                    ...
                    for role in filler_roles:
                        ...
                        if role != "Email_Box" and used >= 360:
                            continue
                        # Lower score is better: prefer roles they are weighted for.
...
3. Optional: add task **weights (0–5)** per staff member to prefer stronger/faster people for specific tasks (e.g., Docman vs EMIS)
4. Upload the completed template and click **Generate**

**Weight columns (optional):** PhonesWeight, BookingsWeight, EmisWeight, DocmanWeight, AwaitingWeight, EmailWeight, TriageWeight, FrontDeskWeight  
Higher = more preferred for that task. Missing/blank = neutral.
""")

# Starter template download (works on Streamlit Cloud when the file is in your repo root)
template_paths = ["rota_generator_template.xlsx", "/mnt/data/rota_generator_template.xlsx"]
tpl_bytes = None
...
if tpl_bytes:
    ...
    st.info("Starter template not found in the app folder. Add 'rota_generator_template.xlsx' to your repo root to enable the download button.")
